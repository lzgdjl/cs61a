...
def analyze_tweet_sentiment(tweet):
    ...
    sum_num,num = 0,0
    for item in words:
    	if has_sentiment(get_word_sentiment(item)):
    		flag = True
    		sum_num += sentiment_value(get_word_sentiment(item))
    		num += 1
    if flag:
    	return make_sentiment(sum_num / num)
...
def distance(location1,location2):
	return abs(square(latitude(location1) - latitude(location2)) - 
	square(longitude(location1) - longitude(location2)))

def find_nearest_location(tweet,state_location_list):
	nearest = 'CA'
	distance_list = {key : distance(tweet_location(tweet),value) for key value in state_location_list.items()}
	for key in distance_list.keys()
		if distance[nearest] > distance[key]:
			nearest = key
	return nearest
...
def group_tweets_by_state(tweets):
    ...
    "*** YOUR CODE HERE ***"
    result = {}
    near_location = {key:find_state_center(value) for key,value in us_states}
	for tweet in tweets:
		key = find_nearest_location(tweet,near_location)
		if key not in result:
			result[key] = [tweet]
		else:
			result[key].append(tweet)
	return result 

def has_none_sentiment(tweets):
	if len(tweets):
		return False
	elif not has_sentiment(analyze_tweet_sentiment(tweets[0])):
		return True
	else:
		return has_none_sentiment(tweets[1:]) 
...
def average_sentiments(tweets_by_state):
    ...
    for key,value in tweets_by_state.items():
    	if len(value) == 0:
    		...
    		temp = keep_if(has_sentiment,apply_to_all(analyze_tweet_sentiment,value))
    		if len(temp) == 0:
    			continue
    		else:
    			avg = average(apply_to_all(sentiment_value,temp))
    			result[key] = avg
    return result
##########################
